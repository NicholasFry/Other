...
def main():
    # Borehole dimensions
    # -------------------
    H = 91.44  # Borehole length (m)
    D = 2.  # Borehole buried depth (m)
    r_b = 0.060325  # Borehole radius (m)
    ...
    pipe_coaxial = \
        plat.media.Pipe(pos_coaxial, r_inner, r_outer, 0, epsilon, k_p_coax,
                        rhoCp_p)
    # Soil
    ugt = 12.5  # Undisturbed ground temperature (degrees Celsius)
    ...
    max_EFT_allowable = 29.44444  # degrees Celsius
    min_EFT_allowable = -1  # degrees Celsius
    # Maximum and minimum allowable heights
    max_Height = 121.92  # in meters
    ...
    hourly_extraction: dict = \
        pd.read_csv('demand_place.csv').to_dict('list')
